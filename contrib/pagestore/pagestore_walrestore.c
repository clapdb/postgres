...
client_attach(const char *shm_name, uint32_t page_size_unused)
{
	 ...
	 * call) rather than failing immediately: restore_command invocations
	 * during recovery must not spuriously fail just because an operator
	 * happened to run `pagestore_inspect relation` at the same moment.
	 */
	{
