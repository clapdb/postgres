...
PR fixing the PR #262 review finding) is not an envelope migration under this
decision: `ForkMetaRecV2`/FKM3 layout, kinds, and the snapshot payload format
are unchanged, and `pagestore_format_versions` reports the same identities
before and after.  It is a content-level recovery rule for records a shipped
bug wrote -- a live ordered write's in-memory marker degraded to a plain GROW
across a second same-lifetime cutover -- explicit and logged, not a silent
tolerance of a new envelope shape.  It is exercised by
`test_orphaned_ordered_marker_adopted()` and
`test_orphaned_ordered_marker_not_adopted_on_mismatch()`
(`pagestore_forkmeta_cutover_test.c`) and by `integration_test.sh`'s clean-
shutdown reopen of its own retained store, which asserts zero adoptions.
