...
read-only `private-test-ipc` inspection schema v3 contract for aggregate health,
...
protocol version 44 and schema version 3.  The minimal H1 relation operation
...
parameter validation, as-of existence/fork-size results, and explicit
`selected_version: null` when the core cannot prove one aggregate version.
...
| 2026-09-06 | Added the minimal H1 relation inspection slice: protocol 44/schema 3, a dedicated private request/response mailbox with daemon-instance, generation, timeout, and concurrent-client fencing, strict relation-only read validation, coherent all-shard as-of existence/fork nblocks, and explicit unavailable selected version; advertised and probed it in C/Python tests | POSIX standalone plus Python schema/runtime coverage; no SPDK execution |
| 2026-09-06 | Hardened H1 relation inspection follow-up: protocol 44, POSIX fd ownership lock across the complete inspector transaction, direct release-published REQUEST without CLAIMED, bounded abandoned-slot recovery, and strict main-fork/existence consistency validation | Focused POSIX mailbox coverage plus standalone/Python tests; no SPDK execution |
| 2026-09-06 | Closed H1 relation-mailbox ownership gaps: byte-zero initialization/client gate, byte-one daemon lifetime lease acquired after byte zero and retained on the shm fd through shutdown, lease-gated stale REQUEST/BUSY recovery, and real fork/SIGKILL lock coverage | POSIX mailbox, standalone, and Python harness tests; no SPDK execution |
