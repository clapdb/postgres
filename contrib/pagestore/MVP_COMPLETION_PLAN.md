...
acknowledged size.  The composed `forkmeta` workload's post-cutoff trickle
is tracked by its oracle: the seed enters every acknowledged create and
growth in a ledger (`--ack-file`, fsynced before the next request; the
first eight relations are acknowledged while maintenance is still paused,
so the ledger is never empty at the crash), and the verify after
recovery, and again after the additional restart, requires every
acknowledged relation to exist at exactly its acknowledged size.  SPDK
...
| 2026-09-13 | Closed the R4b concurrency clause: the crash matrix's deterministic concurrent appender runs at every publication boundary (prepare, manifest commit, source rewrite, snapshot GC) and recovery must show its acknowledged append at exactly the acknowledged size; the composed `forkmeta` workload records every acknowledged trickle create and growth in a ledger (`--ack-file`, the first eight relations acknowledged before maintenance may run) and its verify oracle holds recovery, and the additional restart, to each of them exactly once | `pagestore_forkmeta_crash_matrix_test` (253 checks, five consecutive runs), the four composed forkmeta scenarios (58--66 acknowledged appends verified each, twice), harness unit tests, standalone suite |
