...
controller's own trigger; the reclaimer now watches the specific blocking
page (key, block, LSN window) and re-issues its request from that exact
event alone (`walidx_reclaim_base_epoch`), bounded to at most one
publication per memtable flush that makes such a version durable.  (2)
...
and the note that sets it is superseded, so the next compaction pass prunes
it promptly.  Neither residual is exercised by the soak's own workload, so
its acceptance bounds are unchanged; see the new reclaim-core test cases
below.  Nothing remains before the gate closes beyond keeping the nightly
soak green.
