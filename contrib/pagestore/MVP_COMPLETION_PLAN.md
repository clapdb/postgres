...
Status: **read-only aggregate and per-timeline inspection foundation complete;
relation inspection remains planned for H1; composed H1 scenarios remain**.
...
read-only `private-test-ipc` inspection schema v2 contract for aggregate health,
...
protocol version 43.  Relation inspection is retained in the schema as a
planned, non-advertised H1 operation; the composed H1 scenarios remain
outstanding.
...
| 2026-09-06 | Completed the H0b read-only aggregate and per-timeline inspection foundation: bounded seqlock snapshots and strict `private-test-ipc` schema v2 operations for timeline, manifest, GC, owners, health, backpressure, and pruning; retained relation inspection as a planned, non-advertised H1 operation; bumped runtime protocol advertisements to 43, added strict boolean/nonnegative-counter response typing including explicit unavailable PAGE-debt state, and made runtime probing cover every advertised operation | POSIX standalone integration and Python harness coverage; no SPDK execution |
