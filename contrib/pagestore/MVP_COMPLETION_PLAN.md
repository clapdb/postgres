...
requests an on-demand compacted WAL-index publication for a segment blocked
only by that stale floor, and the backoff is cancelled by a proof-epoch bump
on every event that can move a proof input rather than only by the clock; the
...
20260909 at 8000 rounds now peaks at ~1.75 MiB, stable across CPU regimes;
nothing remains before the gate closes beyond keeping the nightly soak green.
