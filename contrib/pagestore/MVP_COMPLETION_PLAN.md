...
finding and its follow-up review findings F2/F3/F5) is not an envelope
...
freeze sequence covers the record's admission sequence, which -- because the
admission write lock that computes that freeze sequence drains every append
holding the admission read lock across `append_page_impl()` -- can only be
true for a marker that was durably appended and later lost, never for a torn
append still in flight) for records a shipped bug wrote (a live ordered
write's in-memory marker degraded to a plain GROW across a second
same-lifetime cutover, now fixed at the live path) or a still-open finding
degrades (F3: a pruned marker's record rescanned after a timeline-delete
watermark rebase; `RELEASE_VALIDATION.md`).  It is exercised by
