...
/*
 * A refused artifact BEGIN/COMMIT/DROP/WRITE used to be silent: the daemon
 * mapped it straight to PS_STATUS_ERROR and the client's op number was the
 * only clue (see the reader-snapshot key-collision postmortem in
 * RELEASE_VALIDATION.md).  Log one line per refusal and hand the reason back
 * to the client in ch->result, so a CI failure like that is diagnosable from
 * the one run.  block is UINT32_MAX for begin/commit/drop, which have no
...
handle_request(PsChannel *ch)
{
	...
	switch ((PsOpcode) ch->opcode)
	{
		...
		case PS_OP_EXTEND:
			{
				PsArtifactRefuseReason reason = PS_ARTIFACT_REFUSE_NONE;
				int			artifact_klass = ch->key.klass == PS_KLASS_SLRU ||
				...
				if (ps_artifact_write(tl, &ch->key, ch->blocknum, ch->data,
								ch->req_lsn, ch->req_seq, &ch->req_seq, &reason) != 0)
				{
					ch->status = PS_STATUS_ERROR;
					if (artifact_klass)
					{
