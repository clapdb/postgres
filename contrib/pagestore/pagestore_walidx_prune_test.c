...
main(void)
{
...

	{
		...
		check(ps_walidx_prune_plan_bases(items, 6, base60, 1, 80, NULL, 0,
										 keep) == 3 &&
			  !keep[0] && !keep[1] && !keep[2] && keep[3] && keep[4] && keep[5],
			  "a stored image covers the FPI and every earlier record");
		check(ps_walidx_prune_plan_bases(items, 6, base80, 1, 80, NULL, 0,
										 keep) == 2 &&
			  !keep[3] && keep[4] && keep[5],
			  "an image at the cutoff leaves only the future tail");
		check(ps_walidx_prune_plan_bases(no_base, 2, base20, 1, 40, NULL, 0,
										 keep) == 1 && !keep[0] && keep[1],
			  "a page without any FPI compacts from a stored image");
		check(ps_walidx_prune_plan_bases(items, 6, base_future, 1, 80, NULL, 0,
										 keep) == 4 && keep[2],
			  "an image after the cutoff is not yet visible");
		check(ps_walidx_prune_plan_bases(items, 6, base60, 1, 100,
										 (uint64_t[]) {40}, 1, keep) == 4 &&
			  keep[0] && keep[1] && !keep[2] && !keep[3] && keep[4] && keep[5],
			  "an older horizon below the image still keeps its FPI chain");
		check(ps_walidx_prune_plan_bases(items, 6, base60, 1, 100,
										 (uint64_t[]) {80}, 1, keep) == 3 &&
			  !keep[2] && keep[3] && keep[4] && keep[5],
			  "a horizon above the image starts its chain after the image");
		check(ps_walidx_prune_plan_bases(items, 6, unsorted_bases, 2, 80, NULL,
										 0, keep) == -1,
			  "unsorted bases fail closed");
		check(ps_walidx_prune_plan_bases(items, 6, zero_base, 1, 80, NULL, 0,
										 keep) == -1,
			  "a zero base fails closed");
		check(ps_walidx_prune_plan_bases(items, 6, NULL, 0, 80, NULL, 0,
										 keep) == 4,
