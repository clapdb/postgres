...
static int
spdk_wal_truncate(uint32_t tl, uint64_t len)
{
	return PsStoragePosix.wal_truncate(tl, len);
}
...
static int
spdk_walidx_epoch_gc(uint32_t tl, const uint64_t *keep_epochs,
					uint32_t nshards)
{
	return PsStoragePosix.walidx_epoch_gc(tl, keep_epochs, nshards);
}
...
static int
spdk_fork_meta_truncate(uint64_t len)
{
