...
ps_control_drain(void)
{
	...
	while (ps_control_queue_count > 0)
	{
		 ...
		 * object carries this image's checkpoint redo pointer at the same
		 * version LSN.  The daemon derives its durable WAL retention floor
		 * from these notes (wal_retain_floor); writing the note before the
		 * image keeps the invariant conservative -- every restorable image
		 * has a note, and a crash between the two leaves only a note, which
		 * merely retains WAL a little longer.
		 */
		memset(page, 0, sizeof(page));
		memcpy(page, &p->image.checkPointCopy.redo, sizeof(XLogRecPtr));
		pagestore_localsvc_obj_write(PS_KLASS_CONTROL, &key, 1, page,
									 (uint64) p->update_lsn);
