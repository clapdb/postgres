...
| Per-page WAL index and PostgreSQL `rm_redo` reuse | Live index, durable multi-shard snapshot/log-epoch cutover, old generation/epoch GC, persisted known/FPI plus record-end metadata, and the pure replacement-chain planner implemented; durable frontier/cutover integration remains | WAL redo demos plus snapshot publication, generation/epoch GC, replacement-base metadata/planner, tail-replay, and restart tests |
