...
| Per-page WAL index and PostgreSQL `rm_redo` reuse | Live index, durable multi-shard snapshot/log-epoch cutover, and old generation/epoch GC implemented; replacement-base entry compaction remains | WAL redo demos plus snapshot publication, generation/epoch GC, tail-replay, and restart tests |
